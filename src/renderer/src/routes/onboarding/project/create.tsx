...
import { _getByteCount, _getGraphemeCount } from 'valibot'
...
function CreateProjectScreenComponent() {
	...
	const graphemeCount = _getGraphemeCount(projectName.trim())
...
function checkForError(value: string, maxGraphemes: number): boolean {
	if (value.length === 0) {
		return true
	}
	const graphemes = _getGraphemeCount(value)
	const byteLength = _getByteCount(value)
